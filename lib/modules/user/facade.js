...
const getUser = async (info) => {
   try {
     let user = await userService.getOne({_id : info?.userId,status : true,isDeleted : false},{status:0,isDeleted:0,__v:0,createdAt:0,updatedAt:0,lastLogin:0});
     if(!user) throw customException.completeCustomException("user_not_found");
     else return user;
   } catch (error) {
     throw error;
